...
const showJadwal = (hari, jamke) => {
    ...
    if (page.props.jadwals.length > 0 ) {
        let jadwal = page.props.jadwals.find(item => item.hari == hari && (jamke >= parseInt(item.jamke) && jamke < (parseInt(item.jamke) + parseInt(item.jml_jam))))
...
const jams = computed(() => {
    ...
    for (let i = 0; i < 10;i++) {
        res.push(dayjs('2024-02-27 07:00').add((35*i), 'minute').format('HH:mm'))
    }
    ...
    <div class="w-full relative">
        <div class="toolbar h-10 bg-slate-200 flex items-center justify-between p-3 text-slate-800">
            ...
            <div class="items flex gap-4">
                <button @click="cetak" class="hover:bg-slate-600 rounded-full p-1 transition-all">
                    <Icon icon="mdi:printer" class="text-xl hover:text-slate-200 active:text-slate-100" />
                </button>
                <button @click="showForm = true" class="hover:bg-slate-600 rounded-full p-1 transition-all">
