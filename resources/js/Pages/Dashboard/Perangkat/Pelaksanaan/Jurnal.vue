...
<DashLayout title="Jurnal Pembelajaran">
    <el-card>
        ...
        <div class="card-body font-serif cetak">
            <div class="paper w-full bg-slate-100 mb-4 p-4 print:bg-white break-inside-avoid" v-for="(prota, p) in page.props.protas" :key="p">
                <Kop />
                ...
                <div class="identitas">
                    <table>
                        ...
                        <tr>
                            ...
                            <td class="px-1">:</td>
                            <td>{{page.props.tapel.label}} | {{prota.semester}}</td>
                        </tr>
                    </table>
                </div>
                <div class="konten my-6">
                    <table class="w-full">
                        <thead>
                            <tr>
                                <th class="border border-black p-2 bg-slate-200">Hari/Tgl</th>
                                <th class="border border-black p-2 bg-slate-200">Pembelajaran Ke</th>
                                <th class="border border-black p-2 bg-slate-200">ATP</th>
                                <th class="border border-black p-2 bg-slate-200">Materi Pokok</th>
                                <th class="border border-black p-2 bg-slate-200">Kegiatan Pembelajaran</th>
                                <th class="border border-black p-2 bg-slate-200">Asesmen</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <td class="border border-black p-1 align-top">{{dayjs(prota.tanggal).format('dddd, DD/MM/YYYY')}}</td>
                                <td class="border border-black p-1 align-top">{{prota.atp?.materi ?? prota.atp_id}}</td>
                                <td class="border border-black p-1 align-top">{{prota.atp?.tps ?? prota.atp_id}}</td>
                                <td class="border border-black p-1 align-top">{{prota.atp?.konten ?? prota.atp_id}}</td>
                                <td class="border border-black p-1 align-top"></td>
                                <td class="border border-black p-1 align-top">
                                    <div v-if="prota.atp.asesmen" v-html="prota.atp.asesmen" />
    ...
    <el-dialog v-model="formJurnal" fullscreen>
    </el-dialog>
</DashLayout>
</template>