                ...
                <template #header>
                    <div class="toolbar flex justify-between items-center">
                        <h3 class="font-bold text-blue-600 uppercase flex items-center gap-2">
                            <Link :href="appRoute('prosem.index', {_query: params})">
                        ...
                        <table class="my-8 w-full">
                            ...
                            <tbody>
                                <template v-for="(atp, a) in page.props.atps" :key="atp.kode">
                                    <tr v-for="(prosem, p) in atp.prosems" :key="prosem.id">
                                        <td class="border border-black text-center" v-if="a+p == a" :rowspan="(a+p===a ? atp.prosems.length : '')">{{ a+1 }}</td>
                                        <td class="border border-black" v-if="a+p == a" :rowspan="(a+p===a ? atp.prosems.length : '')">{{ atp.materi}}</td>
                                        <td class="border border-black p-2" v-if="a+p == a" :rowspan="(a+p===a ? atp.prosems.length : '')">{{ atp.tps}}</td>
                                        <td class="border border-black p-2" v-if="a+p == a" :rowspan="(a+p===a ? atp.prosems.length : '')">
                                            <span v-html="atp.konten" /></td>
                                        <td class="border border-black text-center" v-if="a+p == a" :rowspan="(a+p===a ? atp.prosems.length : '')">{{ atp.aw}}</td>
                                        <template v-for="m in syahrs" :key="m+'a'">
                                        <td class="border border-black p-2" v-for="w in minggus(m)" :key="w" >
                                            <span v-if="prosem.atp_id == atp.id && prosem.minggu_ke == w && dayjs(prosem.tanggal).format('MMMM') == m">{{ dayjs(prosem.tanggal).format('DD/MM') }}</span>
