...
const kode = (teks, index) => {
    ...
    return params.value.mine == "1"
        ? [split[1], split[2], split[3]].join("") + "-" + (index + 1)
        : split.join("") + "-" + (index + 1);
};
</script>
    ...
    <DashLayout title="Program Tahunan">
        ...
        <el-row class="w-full" v-if="page.props.atps.length > 0">
            <el-col :span="24">
                <el-card :body-style="{ background: '#efefef' }">
                    ...
                    <div
                        class="card-body cetak font-serif w-[33m] print:w-full mx-auto bg-white p-4 shadow print:shadow-none rounded"
                    >
                        <Kop />
                        ...
                        <div class="content my-8">
                            <table class="w-[80%] print:w-[90%] mx-auto">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td
                                            class="border border-black p-2 text-right font-bold bg-slate-200"
                                            colspan="5"
                                        >
                                            JUMLAH
                                        </td>
                                        <td
                                            class="border border-black p-2 text-center font-black"
                                        >
                                            {{
                                                page.props.atps.reduce(
                                                    (a, c) =>
                                                        a + parseInt(c.aw),
                                                    0
                                                )
        ...
        <el-row v-else>
            <el-alert type="error">
                ...
                Susun dulu Alur Tujuan Pembelajaran (ATP)
                <Link
                    class="text-blue-400 font-bold uppercase"
                    :href="
                        appRoute('atp.index', {
                            _query: { fase: params.fase, mine: '1' },
                        })
                    "
                    >Di Sini</Link
                >
