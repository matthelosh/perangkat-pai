...
const onSubmit = async() => {
    router.post(route('menu.store'), {menu: JSON.stringify(menu.value)}, {
        onStart: () => {
            loading.value = true
        },
        onSuccess: (page) => {
...
<Layout title="Pengaturan Menu">
    ...
    <el-table :data="page.props.menus" stripe
        row-key="id">
        ...
        <el-table-column label="Label" >
            <template #default="scope">
                <el-tooltip :content="`Edit Menu ${scope.row.label}`" placement="top-start">
        ...
        <el-table-column label="Opsi">
            <template #default="scope">
                <el-popconfirm :title="`Yakin menghapus menu ${scope.row.label}?`" @confirm="hapus(scope.row.id)">
                    <template #reference>
    ...
    <el-dialog v-model="showDialog" title="Form Menu" @closed="menu={}">
        <el-form v-model="menu" label-width="150px">
            <el-form-item label="Label">
                <el-input type="text" placeholder="Label" v-model="menu.label" />
            </el-form-item>
            <el-form-item label="URL/Route">
                <el-input type="text" placeholder="URL/Route" v-model="menu.url" />
            </el-form-item>
            <el-form-item label="Role">
                <el-select v-model="menu.roles" multiple placeholder="Role Pengguna">
                    <el-option v-for="role in roles" :key="role" :value="role">{{ role }}</el-option>
                </el-select>
            </el-form-item>
            <el-form-item label="Icon">
                <el-input type="text" placeholder="Icon" v-model="menu.icon">
                    <template #prefix>
            ...
            <el-form-item label="Menu Induk">
                <el-select v-model="menu.parent_id" placeholder="Induk">
                    ...
                    <el-option v-for="induk in roots" :key="induk.id" :value="induk.id">{{ induk.label }}</el-option>
                </el-select>
            </el-form-item>
            <el-form-item>
                <el-button type="primary" :loading="loading" @click="onSubmit">Simpan</el-button>
            </el-form-item>
        </el-form>
    </el-dialog>
</Layout>
